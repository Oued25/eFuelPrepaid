...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_compagnie'])) {
    ...
    $stmt = $connection->prepare("SELECT nom_compagnie FROM compagnie WHERE id = :id");
    ...
    if (isset($_SESSION['pointofsale'])) {
        ...
        if (!in_array(filter_input(INPUT_GET, 'id'), $produit_ids)) {
            ...
            $_SESSION['pointofsale'][$count] = array(
                'id' => filter_input(INPUT_GET, 'id'),             // Identifiant du produit
                'nom_produit' => filter_input(INPUT_POST, 'nom_produit'), // Nom du produit
                'prix_achat' => filter_input(INPUT_POST, 'prix_achat'),   // Prix d'achat du produit
                'quantite' => filter_input(INPUT_POST, 'quantite')        // Quantité ajoutée
                //'id_compagnie' => filter_input(INPUT_POST, 'id_compagnie')        // 
                //'nom_compagnie' => filter_input(INPUT_POST, 'nom_compagnie')        // 
            );   
        } else {
        ...
        $_SESSION['pointofsale'][0] = array(
            ...
            'quantite' => filter_input(INPUT_POST, 'quantite')        // Quantité ajoutée
           // 'id_compagnie' => filter_input(INPUT_POST, 'id_compagnie')        // 
            //sss'nom_compagnie' => filter_input(INPUT_POST, 'nom_compagnie')        // 
        );
    }
}
