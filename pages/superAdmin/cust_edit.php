<?php
 include '../../includes/connection.php'; 
 include '../../includes/sidebar.php';

    $id_compagnie= $_GET['id'];

    // Requête SQL avec PDO
    $sql = "SELECT id, code_compagnie, nom_compagnie, contact, adresse, localisation, compte_paiement, date_creation
            FROM compagnie
            WHERE  id = :id"; 

    // Préparation de la requête
    $req = $connection->prepare($sql);  

    // Liaison des paramètres
    $req->bindParam(':id', $id_compagnie);

    // Exécution de la requête
    $req->execute();

    // Vérification des erreurs de requête
    if (!$req) {
        die("Erreur dans la requête SQL: " . $connection->error);
    }

    // Récupération des résultats
    while ($row = $req->fetch(PDO::FETCH_ASSOC)) {
      $zz = $row['id'];
      $zzz = $row['code_compagnie'];
      $B = $row['nom_compagnie'];
      $C = $row['contact'];
      $D = $row['adresse'];
      $E = $row['localisation'];
      $F = $row['compte_paiement'];  
    }

?>
            <center><div class="card shadow mb-4 col-xs-12 col-md-8 border-bottom-primary">
            <div class="card-header py-3">
              <h4 class="m-2 font-weight-bold text-primary">Modifier Compagnie</h4>
            </div><a  type="button" class="btn btn-primary bg-gradient-primary btn-block" href="customer.php?"> <i class="fas fa-flip-horizontal fa-fw fa-share"></i> Retour</a>
            <div class="card-body">
         
            <form role="form" method="post" action="cust_edit1.php">
            <input type="hidden" name="id" value="<?php echo $zz; ?>" />

              <div class="form-group row text-left text-warning">
                    <div class="col-sm-3" style="padding-top: 5px;">
                    code Compagnie:
                    </div>
                    <div class="col-sm-9">
                      <input class="form-control" placeholder="Code Compagnie" name="code_compagnie" value=" <?php echo $zzz; ?>" required>
                    </div>
              </div>

              <div class="form-group row text-left text-warning">
                    <div class="col-sm-3" style="padding-top: 5px;">
                        Nom Compagnie:
                    </div>
                    <div class="col-sm-9">
                        <input class="form-control" placeholder="Nom Compagnie" name="nom_compagnie" value="<?php echo $B; ?>" required>
                    </div>
              </div>

              <div class="form-group row text-left text-warning">
                    <div class="col-sm-3" style="padding-top: 5px;">
                        Adresse:
                    </div>
                    <div class="col-sm-9">
                        <input class="form-control" placeholder="Adresse" name="adresse" value=" <?php echo $D; ?>" required>
                    </div>
              </div>

              <div class="form-group row text-left text-warning">
                    <div class="col-sm-3" style="padding-top: 5px;">
                        Téléphone:
                    </div>
                    <div class="col-sm-9">
                        <input class="form-control" placeholder="Téléphone" name="contact" value=" <?php echo $C; ?>" required>
                    </div>
              </div>

              <div class="form-group row text-left text-warning">
                    <div class="col-sm-3" style="padding-top: 5px;">
                    Localisation:
                    </div>
                    <div class="col-sm-9">
                      <input class="form-control" placeholder="Localisation" name="localisation" value=" <?php echo $E; ?>" required>
                    </div>
              </div>

              <div class="form-group row text-left text-warning">
                  <div class="col-sm-3" style="padding-top: 5px;">
                  Compte Paiement:
                  </div>
                  <div class="col-sm-9">
                    <input class="form-control" placeholder="Compte Paiement" name="compte_paiement" value=" <?php echo $F; ?>" required>
                  </div>
              </div>

              <!--<div class="form-group row text-left text-warning">
                    <div class="col-sm-3" style="padding-top: 5px;">
                        Date Modification:
                    </div>
                    <div class="col-sm-9">
                    <input type="text" onfocus="(this.type='date')" onblur="(this.type='text')" class="form-control" placeholder=" Date Modification" name="date_modif" required>
                    </div>
                </div>-->
              <hr>
                <button type="submit" class="btn btn-warning btn-block"><i class="fa fa-edit fa-fw"></i>Modifier</button> 
              </form>  
          </div>
  </div>

  <?php include '../../includes/footer.php'; ?>